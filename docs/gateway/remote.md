...
- `openclaw health` and `openclaw status --deep` now reach the remote gateway via `wss://0.0.0.0:18789`.
...
When the gateway is loopback-only, keep the URL at `wss://0.0.0.0:18789` and open the SSH tunnel first.
...
- Forward `18789` over SSH (see above), then connect clients to `wss://0.0.0.0:18789`.
