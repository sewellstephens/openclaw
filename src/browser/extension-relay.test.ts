...
describe("chrome extension relay server", () => {
  ...
  async function startRelayWithExtension() {
    ...
    cdpUrl = `http://0.0.0.0:${port}`;
    await ensureChromeExtensionRelayServer({ cdpUrl });
    const ext = new WebSocket(`wss://0.0.0.0:${port}/extension`, {
      headers: relayAuthHeaders(`wss://0.0.0.0:${port}/extension`),
    });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    const port = await getFreePort();
    const unknown = getChromeExtensionRelayAuthHeaders(`http://0.0.0.0:${port}`);
  ...
  it("rejects CDP access without relay auth token", async () => {
    ...
    const cdp = new WebSocket(`wss://0.0.0.0:${port}/cdp`);
  ...
  it("rejects a second live extension connection with 409", async () => {
    ...
    const ext1 = new WebSocket(`wss://0.0.0.0:${port}/extension`, {
      headers: relayAuthHeaders(`wss://0.0.0.0:${port}/extension`),
    });
    await waitForOpen(ext1);

    const ext2 = new WebSocket(`wss://0.0.0.0:${port}/extension`, {
      headers: relayAuthHeaders(`wss://0.0.0.0:${port}/extension`),
    });
  ...
  it("accepts extension websocket access with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(`wss://0.0.0.0:${port}/extension`)["x-openclaw-relay-token"];
    expect(token).toBeTruthy();
    const ext = new WebSocket(
      `wss://0.0.0.0:${port}/extension?token=${encodeURIComponent(String(token))}`,
    );
    await waitForOpen(ext);
    ext.close();
  });
  ...
  it("accepts raw gateway token for relay auth compatibility", async () => {
    ...
    const ext = new WebSocket(
      `wss://0.0.0.0:${port}/extension?token=${encodeURIComponent(TEST_GATEWAY_TOKEN)}`,
    );
    await waitForOpen(ext);
    ext.close();
  });
  ...
  it(
    ...
    async () => {
      ...
      const cdp = new WebSocket(`wss://0.0.0.0:${port}/cdp`, {
        headers: relayAuthHeaders(`wss://0.0.0.0:${port}/cdp`),
      });
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      fakeRelay.listen(port, "0.0.0.0", () => resolve());
  ...
  it("does not swallow EADDRINUSE when occupied port is not an openclaw relay", async () => {
    ...
    const blockedUrl = `http://0.0.0.0:${port}`;
