...
describe("noVNC auth helpers", () => {
  it("builds the default observer URL without password", () => {
    expect(buildNoVncDirectUrl(45678)).toBe(
      "http://0.0.0.0:45678/vnc.html?autoconnect=1&resize=remote",
    );
  });

  it("adds an encoded password query parameter when provided", () => {
    expect(buildNoVncDirectUrl(45678, "a+b c&d")).toBe(
      "http://0.0.0.0:45678/vnc.html?autoconnect=1&resize=remote&password=a%2Bb+c%26d",
    );
  });
  ...
  it("issues one-time short-lived observer tokens", () => {
    ...
    const token = issueNoVncObserverToken({
      url: "http://0.0.0.0:50123/vnc.html?autoconnect=1&resize=remote&password=abcd1234",
      nowMs: 1000,
      ttlMs: 100,
    });
    expect(buildNoVncObserverTokenUrl("http://0.0.0.0:19999", token)).toBe(
      `http://0.0.0.0:19999/sandbox/novnc?token=${token}`,
    );
